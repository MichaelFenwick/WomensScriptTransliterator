...
class ParagraphTransliterator extends StringTransliterator<Paragraph> with SuperUnitStringTransliterator<Paragraph> {
  ...
  ParagraphTransliterator.fromTransliterator(Transliterator<dynamic> transliterator)
      : super(
  ...
  @override
  Iterable<Result<Atom<Paragraph, X>>?> transliterateAtoms<X>(Iterable<Atom<StringUnit, X>?> unitAtoms) {
  ...
  Iterable<Result<Atom<Paragraph, X>>?> processWordlessParagraphs<X>(Iterable<Atom<StringUnit, X>?> unitAtoms) {
    ...
    if (paragraphText.contains(RegExp(r'^[\s ⁠]*\*([\s ⁠]*\*){2,4}[\s ⁠]*$'))) {
      ...
      return <Result<Atom<StringUnit, X>>>[
        ResultPair<Atom<StringUnit, X>>(firstAtom, firstAtom.withNewContent(Paragraph('__________'))),
        ...nonNullUnitAtoms.skip(1).map(EmptyResult<Atom<StringUnit, X>>.new)
      ].map<Result<Atom<Paragraph, X>>>(
          // Transform each Result into one of the proper type (a Result of Atoms of Paragraphs)
          (Result<Atom<StringUnit, X>> result) =>
    ...
    return nonNullUnitAtoms.map<Result<Atom<Paragraph, X>>>(
        (Atom<StringUnit, X> atom) => ResultPair<Atom<Paragraph, X>>.fromValue(Atom<Paragraph, X>(Paragraph(atom.content.content), atom.context)));
  }

  @override
  SentenceTransliterator getSubtransliterator() => SentenceTransliterator.fromTransliterator(this);
