...
class WordTransliterator<S extends Script, T extends Script> extends StringTransliterator<Word, S, T> {
  ...
  ResultPair<Word, S, T> getBestResult(Word input) {
    ...
    return ResultPair<Word, S, T>(input, Word(transliteratedWord.toString()));
  }
  ...
  Result<Word, S, T> getFullResult(Word input) =>
      Result<Word, S, T>.fromIterable(input, _getSubwordTransliterations(input.content, _getOptionSetMapForString(input.content)).toSet().map(Word.new));
