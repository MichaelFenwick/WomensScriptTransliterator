...
class FileDictionary<S extends Script, T extends Script> extends Dictionary<S, T> {
  ...
  FileDictionary({required File file, this.isUpdatable = false}) {
    _dictionaryFile = file;
    _hashmap = parseEntriesFromString(_dictionaryFile.readAsStringSync());
  }
  ...
  FileDictionary.fromName({required String name, bool isUpdatable = false})
      : this(file: File('$dictionaryDirectory/${Script.getName<S>()}2${Script.getName<T>()}/$name'), isUpdatable: isUpdatable);
