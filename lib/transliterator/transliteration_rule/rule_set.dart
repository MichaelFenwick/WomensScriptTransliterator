...
class RuleSet extends IterableBase<Rule> {
  /// An internal Iterable this for which this class acts as a wrapper.
  final Iterable<Rule> _wordRules;

  const RuleSet(this._wordRules);

  @override
  Iterator<Rule> get iterator => _wordRules.iterator;
  ...
  static RuleSet getRuleSet(Direction direction) {
    final RuleSet? ruleSet = ruleSets[direction];
    if (ruleSet != null) {
      return ruleSet;
    }
  ...
  static Map<Direction, RuleSet> ruleSets = <Direction, RuleSet>{
